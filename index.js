...
const defaultConfig = {
  images: {
    ...
    announce: process.env.ANNOUNCE_IMG || "https://i.ibb.co/0ycpgN5m/uwu.jpg"
  },
  text: {
    open: {
      title: "🟢 UwU Café — OPEN",
...
async function sendToChannel(channelId, embed) {
  ...
  if (!channel || !channel.isTextBased()) {
    throw new Error(`Channel ${channelId} is not a usable text channel.`);
  }
...
const commands = [
  new SlashCommandBuilder()
    ...
    .addUserOption(opt =>
      opt.setName("employee").setDescription("Employee at the counter").setRequired(true)
    )
  ...
  new SlashCommandBuilder()
    ...
    .addUserOption(opt =>
      opt.setName("employee").setDescription("Employee leaving the counter").setRequired(true)
    )
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),
  ...
  new SlashCommandBuilder()
    .setName("review")
    .setDescription("Send a customer review")
    .addUserOption(opt =>
      opt.setName("employee").setDescription("Employee").setRequired(true)
    )
    .addIntegerOption(opt =>
      opt.setName("rating").setDescription("Rating 1–5").setMinValue(1).setMaxValue(5).setRequired(true)
    )
    .addStringOption(opt =>
  ...
  new SlashCommandBuilder()
    .setName("setimage")
    .setDescription("Change a café image")
    .addStringOption(opt =>
      opt
        .setName("type")
    ...
    .addStringOption(opt =>
      opt.setName("url").setDescription("Direct image URL").setRequired(true)
    )
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),

  new SlashCommandBuilder()
    .setName("images")
    .setDescription("Show current café images")
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
].map(cmd => cmd.toJSON());
...
client.on("interactionCreate", async interaction => {
  ...
  try {
    if (interaction.commandName === "open") {
      ...
      await sendToChannel(CHANNELS.status, configuredEmbed("open", { employee }, 0x77DD77));
      await interaction.reply({ content: "Café set to **OPEN**.", flags: 64 });
    }

    if (interaction.commandName === "close") {
      const employee = interaction.options.getUser("employee");
      await sendToChannel(CHANNELS.status, configuredEmbed("close", { employee }, 0xFF6961));
      await interaction.reply({ content: "Café set to **CLOSED**.", flags: 64 });
    }

    if (interaction.commandName === "location") {
      await sendToChannel(CHANNELS.location, configuredEmbed("location", {}, 0xFFB6C1));
    ...
    if (interaction.commandName === "setimage") {
      ...
      try {
        ...
        await interaction.reply({ content: "Invalid image URL.", flags: 64 });
        return;
      }

      cafeConfig.images[type] = url;
      saveConfig();

      await interaction.reply({
        content: `Updated **${type.toUpperCase()}** image.`,
        flags: 64
      });
    }

    if (interaction.commandName === "images") {
      const f = img => img || "Not set";
      await interaction.reply({
        content:
          `**UwU Café Images**\n` +
          `OPEN: ${f(cafeConfig.images.open)}\n` +
          `CLOSED: ${f(cafeConfig.images.close)}\n` +
          `LOCATION: ${f(cafeConfig.images.location)}\n` +
          `REVIEW: ${f(cafeConfig.images.review)}\n` +
          `ANNOUNCE: ${f(cafeConfig.images.announce)}`,
        flags: 64
      });
    }
  } catch (err) {
